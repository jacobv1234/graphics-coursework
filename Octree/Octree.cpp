...
// author: Jacob Vincent inspired by above method
/*
*	Method	: OctreeCollision
*
*	Scope	: public
*
*	Purpose	: Returns true if the provided coordinate is inside a leaf node.
*
*	Returns	: bool
*/
bool COctree::OctreeCollision(float x, float y, float z)
{
	if (m_iLevel >= MAX_DEPTH) //leaf
	{
		// check if the coordinate exists in this box
		return ContainsPoint(x, y, z);
	}
	else
	{
		//recurse on all the children of the current node to find the right one
		for (int i = 0; i < NUM_OF_OCTREE_CHILDREN; i++)
		{
			if (m_pobChildren[i] != nullptr) {
				if (m_pobChildren[i]->ContainsPoint(x, y, z) == true) {
					return m_pobChildren[i]->OctreeCollision(x, y, z);
				}
			}	
		}
	}
...
// returns true if the point exists within this node's bounding box
bool COctree::ContainsPoint(float x, float y, float z) {
	//m_dMinX, m_dMinY, m_dMinZ, m_dMaxX, m_dMaxY, m_dMaxZ
